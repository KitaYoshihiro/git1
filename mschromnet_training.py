...
class MultiboxLoss(object):
    ...
    def _l1_smooth_loss(self, y_true, y_pred):
        ...
        """
        #abs_loss = tf.abs(y_true - y_pred)
        abs_loss = K.backend.abs(y_true - y_pred)
        sq_loss = 0.5 * (y_true - y_pred)**2
        #l1_loss = tf.where(tf.less(abs_loss, 1.0), sq_loss, abs_loss - 0.5)
        l1_loss = K.backend.switch(K.backend.less(abs_loss, 1.0), sq_loss, abs_loss - 0.5)
        return K.backend.sum(l1_loss, -1)
        # return tf.reduce_sum(l1_loss, -1)
    ...
    def _softmax_loss(self, y_true, y_pred):
        ...
        """
        #y_pred = tf.maximum(tf.minimum(y_pred, 1 - 1e-15), 1e-15)
        y_pred = K.backend.maximum(K.backend.minimum(y_pred, 1 - 1e-15), 1e-15)
        #softmax_loss = -tf.reduce_sum(y_true * tf.log(y_pred), axis=-1)
        softmax_loss = -K.backend.sum(y_true * K.backend.log(y_pred), axis=-1)
        return softmax_loss
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        """
        #batch_size = tf.shape(y_true)[0]
        batch_size = K.backend.shape(y_true)[0]
        #num_boxes = tf.to_float(tf.shape(y_true)[1])
        num_boxes = K.backend.cast(K.backend.shape(y_true)[1], dtype='float32')        
        ...
        # num_pos = tf.reduce_sum(y_true[:, :, -4], axis=-1) # 教師データ中でpositiveとマークされているprior_boxの数を数える
        num_pos = K.backend.sum(y_true[:, :, -4], axis=-1) # 教師データ中でpositiveとマークされているprior_boxの数を数える
        # pos_loc_loss = tf.reduce_sum(loc_loss * y_true[:, :, -4], axis=1) # y_true の乗算ではTrue=1, False=0 を前提としている
        pos_loc_loss = K.backend.sum(loc_loss * y_true[:, :, -4], axis=1) # y_true の乗算ではTrue=1, False=0 を前提としている
        # pos_conf_loss = tf.reduce_sum(conf_loss * y_true[:, :, -4], axis=1)
        pos_conf_loss = K.backend.sum(conf_loss * y_true[:, :, -4], axis=1)

        # get negatives loss, we penalize only confidence here
        # num_neg = tf.minimum(self.neg_pos_ratio * num_pos, num_boxes - num_pos)
        num_neg = K.backend.minimum(self.neg_pos_ratio * num_pos, num_boxes - num_pos)
        # pos_num_neg_mask = tf.greater(num_neg, 0)
        pos_num_neg_mask = K.backend.greater(num_neg, 0)
        # has_min = tf.to_float(tf.reduce_any(pos_num_neg_mask))
        ...
        num_neg_batch = K.backend.cast(num_neg_batch, dtype='int32')
